...
const AuthScreen = ({ auth, showMessageModal, onAuthSuccess, createUserWithEmailAndPassword, signInWithEmailAndPassword }) => {
    const [isLoginMode, setIsLoginMode] = useState(true); // true for login, false for signup
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    const [loading, setLoading] = useState(false);

    // DEBUG LOGS to inspect props
    useEffect(() => {
        console.log("AuthScreen: auth prop:", auth);
        console.log("AuthScreen: createUserWithEmailAndPassword prop:", createUserWithEmailAndPassword);
        console.log("AuthScreen: signInWithEmailAndPassword prop:", signInWithEmailAndPassword);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isLoginMode) {
                // Check if signInWithEmailAndPassword is truly a function before calling
                if (typeof signInWithEmailAndPassword !== 'function') {
                    throw new Error("signInWithEmailAndPassword is not a function. Check AuthAndGameHandler prop passing.");
                }
                await signInWithEmailAndPassword(email, password); 
                showMessageModal("Logged in successfully!", 'success');
            } else {
                // Check if createUserWithEmailAndPassword is truly a function before calling
                if (typeof createUserWithEmailAndPassword !== 'function') {
                    throw new Error("createUserWithEmailAndPassword is not a function. Check AuthAndGameHandler prop passing.");
                }
            ...
            if (error.code) {
                ...
                errorMessage = error.message; // Catch the custom error thrown above
            }
